...
impl SWQoSTrait for DefaultSWQoSClient {
    async fn send_transaction(&self, transaction: VersionedTransaction) -> anyhow::Result<()> {
        self.swqos_client.send_swqos_transaction(&self.swqos_endpoint, &transaction).await
    }

    async fn send_transactions(&self, transactions: Vec<VersionedTransaction>) -> anyhow::Result<()> {
        self.swqos_client.send_swqos_transactions(&self.swqos_endpoint, &transactions).await
    }
...
impl DefaultSWQoSClient {
    pub fn new(name: &str, rpc_client: Arc<RpcClient>, endpoint: String, tip_accounts: Vec<Pubkey>) -> Self {
