use super::{
    amm_calc::{amm_buy_get_token_out, amm_sell_get_sol_out},
...
pub const PUBKEY_PUMPSWAP_GLOBAL: Pubkey = pubkey!("ADyA8hdefvWN2dbGGWFotbzWxrAvLW83WG6QCVXvJKqw");
pub const PUBKEY_PUMPSWAP_EVENT_AUTHORITY: Pubkey = pubkey!("GS4CU59F31iL7aR2Q8zVS8DRrcRnXX1yjQ66TqNVQnaR");

#[derive(Debug, Clone, BorshSerialize, BorshDeserialize, Serialize, Deserialize)]
pub struct PumpSwapGlobalAccount {
...
pub struct PumpSwap {
    ...
    pub global_account: OnceCell<Arc<PumpSwapGlobalAccount>>,
}

#[async_trait::async_trait]
impl DexTrait for PumpSwap {
    async fn initialize(&self) -> anyhow::Result<()> {
        let account = self.endpoint.rpc.get_account(&PUBKEY_PUMPSWAP_GLOBAL).await?;
        let global_account = bincode::deserialize::<PumpSwapGlobalAccount>(&account.data)?;
    ...
    async fn sell(
        ...
        let token_amount = match token_amount {
            TokenAmountType::Percent(percent) => {
                ...
                let balance = self.endpoint.rpc.get_token_account_balance(&ata).await?;
                let balance_u64 = u64::from_str(&balance.amount)?;
                (balance_u64 * percent) / 100
            }
            TokenAmountType::Amount(amount) => amount,
        };
...
impl PumpSwap {
    ...
    pub async fn get_pool(&self, mint_address: &Pubkey) -> anyhow::Result<(Pubkey, PumpSwapPoolAccount)> {
        let filters = vec![
        ...
        let pool_data = PumpSwapPoolAccount::try_from_slice(&account.data)?;
    ...
    fn build_buy_instruction(
        ...
        Ok(Instruction::new_with_bytes(
            ...
            vec![
                ...
                AccountMeta::new_readonly(PUBKEY_PUMPSWAP_GLOBAL, false),
                ...
                AccountMeta::new_readonly(PUBKEY_PUMPSWAP_EVENT_AUTHORITY, false),
